...
function Configger(config) {
    ...
    log.level = options.silent || options.verbose || options.loglevel;
    ...
    this.types = [{
        type  : "js",
        regex : new RegExp(options.extension)
    }];

    if(options.css) {
        this.filters.push(new RegExp(options.cssfilter));

        this.types.push({
            type   : "css",
            regex  : new RegExp(options.cssextension),
...
Configger.prototype = {
    ...
    _modules : function() {
        ...
        files = shell.find(options.root)
            .filter(function(file) {
                ...
                // More complex checks
                valid && (valid = self.filters.some(function(regex) {
                    return regex.test(file);
                }));
                
                valid && (valid = self.types.some(function(item) {
                    return item.regex.test(path.extname(file));
                }));
        ...
        modules = files
            .map(function(file) {
                ...
                self.types.some(function(item) {
                    if(!item.regex.test(path.extname(file))) {
                        return;
                    }
    ...
    _config : function() {
        var options = this.options,
            config  = shell.find(options.root),
            source;
